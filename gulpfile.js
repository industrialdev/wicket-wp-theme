...
// BrowserSync configuration - initialize only when needed
function initBrowserSync() {
    if (!browserSync.active) {
        browserSync.init({
            ...
            notify: false, // Optional: disable BrowserSync notifications
        });
    }
}
...
function parseJsonTheme(
    ...
    if (
        ...
        obj.settings?.custom?.["border-radius"]
    ) {
        Object.keys(obj.settings.custom["border-radius"]).forEach(function (
            key
        ) {
            scssVariables[`$${mainPrefix}border-radius-${key}`] =
                obj.settings.custom["border-radius"][key];
            cssVariables[`--border-radius-${key}`] =
                obj.settings.custom["border-radius"][key];
        });
    }
    ...
    if (extract === "box-shadow" && obj.settings?.custom?.["box-shadow"]) {
        Object.keys(obj.settings.custom["box-shadow"]).forEach(function (key) {
            scssVariables[`$${mainPrefix}box-shadow-${key}`] =
                obj.settings.custom["box-shadow"][key];
            cssVariables[`--box-shadow-${key}`] =
                obj.settings.custom["box-shadow"][key];
        });
    }
    ...
    if (extract === "layout" && obj.settings?.custom?.layout) {
        Object.keys(obj.settings.custom.layout).forEach(function (key) {
            scssVariables[`$${mainPrefix}layout-${key}`] =
                obj.settings.custom.layout[key];
    ...
    if (
        ...
        obj.settings?.custom?.["letter-spacing"]
    ) {
        Object.keys(obj.settings.custom["letter-spacing"]).forEach(function (
            key
        ) {
            scssVariables[`$${mainPrefix}letter-spacing-${key}`] =
                obj.settings.custom["letter-spacing"][key];
            cssVariables[`--letter-spacing-${key}`] =
                obj.settings.custom["letter-spacing"][key];
        });
    }
    ...
    if (extract === "line-height" && obj.settings?.custom?.["line-height"]) {
        Object.keys(obj.settings.custom["line-height"]).forEach(function (key) {
            scssVariables[
                `$${mainPrefix}line-height-${key}`
            ] = `${obj.settings.custom["line-height"][key]}rem`;
            cssVariables[
                `--line-height-${key}`
            ] = `${obj.settings.custom["line-height"][key]}rem`;
        });
...
function themeJsonToSCSS() {
    return new Promise(function (resolve, reject) {
        ...
        const themeScssPath = path.join(
            wicketPaths.src,
            "assets/styles/variables/_theme-json.scss"
        );
    ...
    return gulp
        ...
        .pipe(
            sass({
                ...
                silenceDeprecations: ["import", "mixed-decls", "legacy-js-api"], // Silence @import, mixed declarations, and legacy JS API deprecation warnings
        ...
        .pipe(browserSync.active ? browserSync.stream() : gulp.dest("./"));
}
...
function minScripts() {
    return gulp
        .src(
            `${wicketPaths.src}${wicketPaths.assets}/scripts/${wicketPaths.script}.js`
        )
        .pipe(plumber({ errorHandler: onError }))
        .pipe(webpack({}))
        ...
        .pipe(uglify())
        .pipe(
            gulp.dest(`${wicketPaths.src}${wicketPaths.assets}/scripts/min/`)
        );
}
...
function watchTask() {
    // Initialize BrowserSync only when watching
    initBrowserSync();
    ...
    gulp.watch(jsLocations, gulp.series(minScripts, minScriptsAdmin)).on(
        "change",
        browserSync.reload
    );
    gulp.watch(scssTailwindLocations, gulp.series(sassTask, minSass)).on(
        "change",
        browserSync.reload
    );
    gulp.watch(
        themeJsonPath,
        gulp.series(themeJsonToSCSS, sassTask, minSass)
